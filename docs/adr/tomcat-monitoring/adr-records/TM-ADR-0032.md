...
Pada domain beban kerja aplikasi Tomcat, kakas [`tcctl`](https://github.com/edkas07-oss/tcctl) telah membuktikan keberhasilan konsep *Super Operator*: satu biner statis Go mandiri yang mencakup deployment blue-green, audit CIS hardening, sertifikat SSL, pemindaian kerentanan (VA), REST API daemon, hingga mesin GitOps mandiri. Sangat janggal apabila domain pemantauan (*monitoring*) justru mempertahankan arsitektur lama yang terfragmentasi.
