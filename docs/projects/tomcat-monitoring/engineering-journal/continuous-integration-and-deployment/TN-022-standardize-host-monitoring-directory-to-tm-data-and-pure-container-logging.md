...
1. **Renaming Direktori Monitoring**: Mengganti penamaan direktori generik `C:\monitoring` menjadi **`C:\tm_data`** (Windows) dan **`/tm_data`** (Linux).
...
| `inventories/group_vars/all.yml` | Mendefinisikan default `tm_root_dir_windows: "C:\tm_data"`, `tm_root_dir_linux: "/tm_data"`, dan relasi dinamis `project_root: "{{ custom_project_root \| default(tm_root_dir) }}"`. |
...
2. **Live AWS EC2 Windows Deployment**:
   - Playbook `playbooks/deploy-windows.yml` sukses diterapkan pada `aws-ec2-win-01` (`ok=79 changed=17 failed=0`).
