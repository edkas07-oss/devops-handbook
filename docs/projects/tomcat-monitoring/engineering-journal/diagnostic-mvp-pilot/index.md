...
16. **[TN-016 — Implement Restricted Collector and Tomcat Runtime](TN-016-implement-restricted-collector-and-tomcat-runtime.md)**

    Mengimplementasikan repository tata kelola Restricted Event Collector
    (`tomcat-diagnostic-event-collector`), logic atomic spooling dan component test,
    skrip deployment Tomcat aktual (`deploy-tomcat.sh`), serta integrasi mount spool
    read-only pada Diagnostic Service.

17. **[TN-017 — Verify End-to-End Incident Diagnostic Flow](TN-017-verify-end-to-end-incident-diagnostic-flow.md)**

    Melakukan pengujian dan verifikasi end-to-end skenario insiden Tomcat down pada
    environment persisten `devops-lab`, membuktikan pencatatan spool atomik, firing
    alert `TomcatDown`, evaluasi branch TD-06, persistensi SQLite, dan pengiriman
    email diagnosis firing serta notifikasi recovery resolved ke Mailpit.

18. **[TN-018 — Implement Strict Declarative Rulepack Engine and Append-Only Rules API](TN-018-implement-strict-declarative-rulepack-engine.md)**

    Mengimplementasikan declarative rulepack engine dan append-only rules API pada
    Diagnostic Service (`POST /api/v1/rules`), 5 lapis ingestion guard, SQLite
